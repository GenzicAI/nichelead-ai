...
import {
  ...
  CalendarCheck, DollarSign, FileText, ShieldCheck, TrendingUp, LayoutGrid,
} from 'lucide-react'
...
function StatCard({ stat, accent, soft }: { stat: { label: string; value: string; delta: string; iconName: string }; accent: string; soft: string }) {
  ...
  return (
    <div className="shrink-0 w-[150px] lg:w-full rounded-2xl bg-white border border-slate-100 p-4"
...
function CampaignsScreen({ niche }: { niche: typeof NICHES[NicheKey] }) {
  ...
  return (
    <div className="px-4 lg:px-8 pt-2 pb-28">
  ...
  return (
    <div className="px-4 lg:px-8 pt-2 pb-28">
      ...
      <div className="mt-8 text-center text-[12px] text-slate-400">
        NicheLead AI is built and operated by Genzic.AI · <a href="https://tanxusa.com" target="_blank" rel="noopener noreferrer" className="underline">TanXUSA.com</a>
      </div>
    </div>
  )
}
...
export default function Dashboard({ initialNiche, onBack }: { initialNiche: NicheKey; onBack: () => void }) {
  ...
  return (
    <div className="min-h-screen w-full flex justify-center" style={{ background: '#EEF1F4' }}>
      <div className="relative w-full bg-[#F6F7F9] flex flex-col max-w-[440px] lg:max-w-[1180px] lg:my-6 lg:rounded-3xl lg:shadow-2xl lg:overflow-hidden"
        style={{ minHeight: '100vh' }}>

        {/* TOP BAR */}
        <header className="sticky top-0 z-30 bg-white/90 backdrop-blur border-b border-slate-100">
          <div className="px-4 lg:px-8 pt-3.5 pb-2 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-[11px] text-slate-400 mt-0.5">{niche.label} workspace · Genzic.AI</div>
              </div>
            </div>
            <div className="flex items-center gap-1.5">
              <button onClick={onBack}
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-slate-500 hover:bg-slate-100 transition-colors">
                <LayoutGrid size={17} />
                <span className="hidden lg:inline text-[12px] font-semibold">Industries</span>
              </button>
          ...
          <div className="px-3 lg:px-8 pb-2.5 flex gap-2 overflow-x-auto lg:flex-wrap lg:overflow-visible" style={{ scrollbarWidth: 'none' }}>
            ...
            <>
              {tab === 'dashboard' && (
                <section className="pt-4">
                  <div className="px-4 lg:px-8 flex gap-3 overflow-x-auto pb-1 lg:grid lg:grid-cols-4 lg:gap-4 lg:overflow-visible">
              ...
              {activeChips.length > 0 && (
                <div className="px-4 lg:px-8 pb-1 flex gap-1.5 flex-wrap">
                  {activeChips.map(c => (
              ...
              <section className="px-4 lg:px-8 pt-2 pb-28 flex flex-col gap-3 lg:grid lg:grid-cols-2 xl:grid-cols-3 lg:gap-4">
                {leads.map(lead => (
        ...
        <nav className="sticky bottom-0 z-30 bg-white/95 backdrop-blur border-t border-slate-100">
          <div className="flex items-stretch justify-between px-2 lg:px-8 py-1.5 lg:max-w-[640px] lg:mx-auto">
            {TABS.map(t => {
